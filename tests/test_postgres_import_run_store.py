...
def test_postgres_selection_uses_environment_owned_connection_factory(monkeypatch):
    ...
    monkeypatch.setattr(import_run_store_factory, "get_postgres_connection_factory", lambda settings: connection_factory)
    monkeypatch.setattr(import_run_store_factory, "PostgresImportRunStore", FakePostgresImportRunStore)
    store = import_run_store_factory.get_import_run_store(Settings(bookmark_store_backend="postgres"))
...
def test_postgres_import_schema_and_updates_are_tenant_scoped():
    ...
    statements.clear()
    store.update_run(import_id="run-1", user_id="tenant-a", fields={"status": "running"}, now="2026-08-31T00:00:00+00:00")
    store.update_item(item_id=7, user_id="tenant-a", status="completed", detail="done", error=None, external_id="ext-1", now="2026-08-31T00:00:01+00:00")
    assert any("WHERE import_id = %s AND user_id = %s" in statement for statement, _ in statements)
    assert any("WHERE id = %s AND user_id = %s" in statement for statement, _ in statements)


def test_sqlite_import_store_blocks_cross_tenant_reads_and_updates(tmp_path):
    settings = Settings(sqlite_path=str(tmp_path / "imports.db"))
    store = ImportRunStore(settings)
    store.create(import_id="run-1", user_id="tenant-a", connector_id="bookmarks.v1", items=[("https://example.com", "Example")], now="2026-08-31T00:00:00+00:00")
    ...
    store.update_item(item_id=item["id"], user_id="tenant-b", status="completed", detail="wrong tenant", error=None, external_id="", now="2026-08-31T00:00:01+00:00")
