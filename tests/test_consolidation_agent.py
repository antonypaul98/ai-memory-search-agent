...
def test_does_not_merge_cross_type_or_cross_tenant_entities(test_settings: Settings) -> None:
    store = get_knowledge_graph_store(test_settings)
    store.upsert_entity(user_id="user-a", entity_type=EntityType.COMPANY, name="Acme Inc")
    store.upsert_entity(user_id="user-a", entity_type=EntityType.PROJECT, name="AcmeInc")
    store.upsert_entity(user_id="user-b", entity_type=EntityType.COMPANY, name="AcmeInc")
    out = ConsolidationAgent(test_settings).analyze(
        user_id="user-a",
        request=ConsolidationRequest(),
    )
...
def test_consolidation_api_uses_authenticated_user(client: TestClient, test_settings: Settings) -> None:
    store = get_knowledge_graph_store(test_settings)
    store.upsert_entity(user_id=LOCAL_DEFAULT_USER_ID, entity_type=EntityType.TECHNOLOGY, name="Postgres")
    store.upsert_entity(user_id=LOCAL_DEFAULT_USER_ID, entity_type=EntityType.TECHNOLOGY, name="Post gres")
    store.upsert_entity(user_id="other", entity_type=EntityType.COMPANY, name="Private Corp")
    store.upsert_entity(user_id="other", entity_type=EntityType.COMPANY, name="PrivateCorp")
...
def test_consolidation_merge_requires_explicit_confirmation_then_applies(
    ...
    store = get_knowledge_graph_store(test_settings)
    target = store.upsert_entity(user_id=LOCAL_DEFAULT_USER_ID, entity_type=EntityType.COMPANY, name="OpenAI")
    source = store.upsert_entity(user_id=LOCAL_DEFAULT_USER_ID, entity_type=EntityType.COMPANY, name="Open AI")
    ...
    missing_confirmation = client.post(
        ...
        json={"target_entity_id": target.entity_id, "source_entity_id": source.entity_id},
    )
    ...
    denied_confirmation = client.post(
        "/api/v1/agents/consolidation/approve-merge",
        json={"target_entity_id": target.entity_id, "source_entity_id": source.entity_id, "confirm": False},
    )
    ...
    approved = client.post(
        "/api/v1/agents/consolidation/approve-merge",
        json={"target_entity_id": target.entity_id, "source_entity_id": source.entity_id, "confirm": True},
    )
...
def test_consolidation_merge_approval_cannot_cross_tenant(
    ...
    store = get_knowledge_graph_store(test_settings)
    target = store.upsert_entity(user_id=LOCAL_DEFAULT_USER_ID, entity_type=EntityType.COMPANY, name="Acme")
    other_source = store.upsert_entity(user_id="other", entity_type=EntityType.COMPANY, name="Acme Inc")

    response = client.post(
        "/api/v1/agents/consolidation/approve-merge",
        json={"target_entity_id": target.entity_id, "source_entity_id": other_source.entity_id, "confirm": True},
    )
