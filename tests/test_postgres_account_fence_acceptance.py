...
def test_late_relational_and_vector_writers_cannot_recreate_erased_state(fenced_pg):
    ...
    fence = AccountErasureFence(factory)
    writes = [
        lambda owner: memories.upsert(user_id=owner, source_type=SourceType.WEB, external_id="shared", canonical_url="https://example.test/shared", title="private memory"),
        lambda owner: captures.create(capture_id=owner, user_id=owner, url="https://example.test", url_hash="hash", title="capture", source_type="web", payload_json="{}", now="2026-09-21T00:00:00Z"),
        lambda owner: imports.create(import_id=owner, user_id=owner, connector_id="test", items=[], now="2026-09-21T00:00:00Z"),
        lambda owner: oauth.put(user_id=owner, connector_id="test", encrypted_payload=b"test-only", scopes_json="[]", expires_at=None, now="2026-09-21T00:00:00Z"),
        lambda owner: events.insert_event(event_id=owner, user_id=owner, event_type="test", aggregate_type="", aggregate_id="", actor="test", request_id=None, payload_json="{}", created_at="2026-09-21T00:00:00Z"),
...
def test_confirmed_account_erasure_blocks_enabled_producers_and_preserves_neighbor(fenced_pg, monkeypatch):
    ...
    owners = [privacy._auth_store.create_user(email=f"{label}@example.test", password="test-password", display_name=label).user_id for label in ("target", "neighbor")]
    target, neighbor = owners
    def writes(owner, nonce):
        return {
            "canonical_ingress": lambda: privacy._memory_store.upsert(user_id=owner, source_type=SourceType.WEB, external_id="source-" + nonce, canonical_url="https://example.test/" + nonce, title="private"),
            "capture": lambda: privacy._capture_store.create(capture_id=owner + nonce, user_id=owner, url="https://example.test", url_hash=nonce, title="private", source_type="web", payload_json="{}", now=now),
            "import_replay": lambda: imports.create(import_id=owner + nonce, user_id=owner, connector_id="test", items=[("https://example.test", "private")], now=now),
            "oauth_callback": lambda: oauth.put(user_id=owner, connector_id="test", encrypted_payload=b"test", scopes_json="[]", expires_at=None, now=now),
            "agent": lambda: agents.create_run(user_id=owner, run_id=owner + nonce, agent_id="test", task="private", tool="search", arguments_json="{}", policy_tier="read_only", status="queued", message="", created_at=now),
            "background_job": lambda: jobs.create_playlist_job(user_id=owner, playlist_id="test", playlist_title="test", entries=[PlaylistVideoEntry("shared", "https://example.test", "private")], reflection=None, force_refresh=False),
    ...
    after = privacy.export_user_data(user_id=neighbor)
    assert after.pop("exported_at")
    assert preserved.pop("exported_at")
