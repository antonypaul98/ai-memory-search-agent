...
def test_offline_queue_does_not_persist_authentication_secrets():
    source = OFFLINE.read_text(encoding="utf-8")
    # Queue records contain only the canonical URL and queue timestamp. The current
    # token is read at replay time, so bearer credentials are never written to IDB.
