...
def test_knowledge_graph_store_defaults_to_sqlite(tmp_path):
    ...
    assert isinstance(knowledge_graph_store_factory.get_selected_knowledge_graph_store(settings), KnowledgeGraphStore)
...
def test_knowledge_graph_schema_preserves_tenant_identity_and_deterministic_indexes():
    ...
    class Conn:
        ...
        def execute(self, sql, params=None): statements.append((" ".join(str(sql).split()), params)); return self
...
def test_entity_reads_and_search_are_exact_tenant_and_deterministic():
    ...
    class Conn:
        def __enter__(self): return self
        def __exit__(self, *args): return False
        def execute(self, sql, params=None): statements.append((" ".join(str(sql).split()), params)); return Result()
    ...
    assert store.search_entities(user_id="tenant-a", query="Python", entity_type=EntityType.TECHNOLOGY, limit=7) == []
...
def test_relation_write_checks_both_entities_belong_to_tenant():
    ...
    with pytest.raises(ValueError, match="not owned by tenant"):
        store.upsert_relation(user_id="tenant-a", subject_entity_id="entity-a", predicate="related_to", object_entity_id="entity-b")
...
def test_memory_links_require_exact_tenant_entity_ownership():
    ...
    statements.clear()
    with pytest.raises(ValueError, match="not owned by tenant"):
        store.link_memory_entity(MemoryEntityLink(memory_id="memory-1", entity_id="entity-1"), user_id="tenant-a")
