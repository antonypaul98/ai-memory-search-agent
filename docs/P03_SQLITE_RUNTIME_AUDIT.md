...
| `services/privacy_service.py` | Privacy deletion of `content_url_index`, YouTube memory export/deletion, capture export, and bookmark export now route through selected tenant-scoped SQLite/Postgres stores. User, background-job and topic-profile export reads plus graph/intelligence, trust/version/lifecycle and capsule deletion still directly access SQLite and must be migrated in bounded slices before production cutover; preserve confirmation and retention contracts. |
