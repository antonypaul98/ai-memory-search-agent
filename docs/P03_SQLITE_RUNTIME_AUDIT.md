...
| `services/memory_intelligence_service.py` | Reads legacy `memory_capsules_json` and `agent_search_events` directly. Capsule reads must follow the selected artifact store and prove tenant identity; do not guess an owner from video ID. |
...
| `db/postgres_*_migration.py`, `fts_retrieval_parity.py` | Intentional legacy source reads. Do not count migration source reads as production writes. Legacy FTS ownership handling and the parity validator's schema-creating Postgres constructor need further hardening. |
...
2. Reroute tenant-bearing capsule reads and `content_url_index` safely; audit privacy export/deletion before live cutover.
