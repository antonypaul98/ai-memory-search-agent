...
from app.db.knowledge_graph_store import (
    _relation_active_at,
...
def _relation(row: Any) -> GraphRelation:
    ...
    return GraphRelation(
        ...
        metadata=metadata,
        valid_from=metadata.get("valid_from") or _iso(row["created_at"]),
...
class PostgresKnowledgeGraphStore:
    ...
    def ensure_schema(self) -> None:
        with self._connection_factory() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_entities_user_order "
                "ON kg_entities(user_id, updated_at DESC, entity_id ASC)"
            )
            conn.execute(
                """
            ...
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS uq_kg_relations_identity "
                "ON kg_relations(user_id, subject_entity_id, predicate, "
                "object_entity_id, COALESCE(memory_id, ''))"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_relations_subject "
                "ON kg_relations(user_id, subject_entity_id, created_at DESC, relation_id ASC)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_relations_object "
                "ON kg_relations(user_id, object_entity_id, created_at DESC, relation_id ASC)"
            )
            conn.execute(
                """
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_memory_entities_memory "
                "ON kg_memory_entities(user_id, memory_id, entity_id)"
            )
        ...
        with self._connection_factory() as conn:
            ...
            if row:
                merged_aliases = sorted(
                    set(json.loads(row["aliases_json"] or "[]") + aliases)
                )
                row = conn.execute(
                    "UPDATE kg_entities SET name=%s, aliases_json=%s, metadata_json=%s, "
                    "updated_at=%s WHERE entity_id=%s AND user_id=%s RETURNING *",
                    (
                        name.strip(),
                ...
                row = conn.execute(
                    ...
                    "(entity_id,user_id,entity_type,name,normalized_name,aliases_json,metadata_json,created_at,updated_at) "
                    "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *",
                    (
                        entity_id or str(uuid.uuid4()),
                        user_id,
                        entity_type.value,
                        name.strip(),
                        normalized,
    ...
    def get_entity(self, entity_id: str, *, user_id: str) -> GraphEntity | None:
        with self._connection_factory() as conn:
            row = conn.execute(
                "SELECT * FROM kg_entities WHERE entity_id = %s AND user_id = %s",
                (entity_id, user_id),
            ).fetchone()
    ...
    def search_entities(
        ...
        *,
        ...
        if query.strip():
            clauses.append("(normalized_name LIKE %s OR name LIKE %s)")
            params.extend(
                [f"%{normalize_entity_name(query)}%", f"%{query.strip()}%"]
            )
        params.append(limit)
        with self._connection_factory() as conn:
            rows = conn.execute(
                f"SELECT * FROM kg_entities WHERE {' AND '.join(clauses)} "
                "ORDER BY updated_at DESC, entity_id ASC LIMIT %s",
                tuple(params),
            ).fetchall()
    ...
    def _require_owned_entity(self, conn: Any, entity_id: str, user_id: str) -> None:
        if (
            conn.execute(
                "SELECT 1 FROM kg_entities WHERE entity_id = %s AND user_id = %s",
                (entity_id, user_id),
            ).fetchone()
            is None
        ):
        ...
        *,
        ...
        with self._connection_factory() as conn:
            ...
            existing = conn.execute(
                "SELECT * FROM kg_relations WHERE user_id=%s AND subject_entity_id=%s "
                "AND predicate=%s AND object_entity_id=%s "
                "AND COALESCE(memory_id,'')=COALESCE(%s,'') FOR UPDATE",
                (
                    user_id,
                    subject_entity_id,
                    predicate.value,
            ...
            if existing:
                ...
                row = conn.execute(
                    ...
                    "(relation_id,user_id,subject_entity_id,predicate,object_entity_id,memory_id,confidence,metadata_json,created_at) "
                    "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *",
                    (
                        str(uuid.uuid4()),
                        user_id,
    ...
    def get_relation(
        self, relation_id: str, *, user_id: str
    ) -> GraphRelation | None:
        with self._connection_factory() as conn:
            row = conn.execute(
                "SELECT * FROM kg_relations WHERE relation_id=%s AND user_id=%s",
                (relation_id, user_id),
            ).fetchone()
    ...
    def close_relation(
        ...
        *,
        ...
        return self.upsert_relation(
            ...
            valid_to=valid_to or _now().isoformat(),
        )
        ...
        *,
        ...
        with self._connection_factory() as conn:
            if direction in ("outgoing", "both"):
                rows.extend(
                    conn.execute(
                        "SELECT * FROM kg_relations WHERE user_id=%s AND subject_entity_id=%s "
                        "ORDER BY created_at DESC, relation_id ASC",
                        (user_id, entity_id),
                    ).fetchall()
                )
            if direction in ("incoming", "both"):
                rows.extend(
                    conn.execute(
                        "SELECT * FROM kg_relations WHERE user_id=%s AND object_entity_id=%s "
                        "ORDER BY created_at DESC, relation_id ASC",
                        (user_id, entity_id),
                    ).fetchall()
                )
        ...
        if at_time is not None:
            relations = [
                ...
                if _relation_active_at(relation, at_time)
            ]
        return relations
    ...
    def link_memory_entity(self, link: MemoryEntityLink, *, user_id: str) -> None:
        with self._connection_factory() as conn:
            ...
            conn.execute(
                ...
                "mention_context=EXCLUDED.mention_context,start_time=EXCLUDED.start_time,"
                "end_time=EXCLUDED.end_time,confidence=EXCLUDED.confidence",
                (
                    link.memory_id,
                    link.entity_id,
                    user_id,
    ...
    def list_memory_entities(
        ...
        with self._connection_factory() as conn:
            rows = conn.execute(
                "SELECT e.* FROM kg_entities e JOIN kg_memory_entities me "
                "ON me.entity_id=e.entity_id AND me.user_id=e.user_id "
                "WHERE me.memory_id=%s AND me.user_id=%s "
        ...
        *,
        ...
        for _ in range(depth):
            ...
            for eid in sorted(frontier):
                for relation in self.list_relations_for_entity(
                    eid, user_id=user_id, direction="both", at_time=at_time
                ):
                    seen_relations[relation.relation_id] = relation
                    other_id = (
                        relation.object_entity_id
                        if relation.subject_entity_id == eid
