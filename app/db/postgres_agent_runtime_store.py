...
class PostgresAgentRuntimeStore:
    """Persist agent runs/tool calls without changing runtime policy semantics."""
    ...
    def __init__(self, connection_factory: ConnectionFactory) -> None:
        ...
        with connection_factory() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_agent_tool_calls_tenant "
                "ON agent_tool_calls(user_id, run_id, id)"
            )

    def create_run(
        self, *, run_id: str, user_id: str, agent_id: str, task: str, tool: str,
    ...
    def get_run(self, *, user_id: str, run_id: str) -> dict[str, Any] | None:
        with self._connection_factory() as conn:
            row = conn.execute(
                "SELECT * FROM agent_runs WHERE run_id=%s AND user_id=%s",
                (run_id, user_id),
            ).fetchone()
    ...
    def set_run_running(self, *, user_id: str, run_id: str, updated_at: str) -> None:
        with self._connection_factory() as conn:
            conn.execute(
                """
                UPDATE agent_runs SET status='running', message='', updated_at=%s
                WHERE run_id=%s AND user_id=%s
                """,
    ...
    def create_tool_call(
        self, *, user_id: str, run_id: str, tool: str,
        arguments_json: str, created_at: str,
    ) -> int:
        with self._connection_factory() as conn:
            row = conn.execute(
                """
    ...
    def mark_failed(
        self, *, user_id: str, run_id: str, call_id: int,
        error: str, updated_at: str,
    ) -> None:
        with self._connection_factory() as conn:
            conn.execute(
                """
                UPDATE agent_tool_calls SET status='failed', error=%s
                WHERE id=%s AND run_id=%s AND user_id=%s
                """,
                (error, call_id, run_id, user_id),
            )
            conn.execute(
                """
                UPDATE agent_runs
                ...
                WHERE run_id=%s AND user_id=%s
                """,
                (error, updated_at, run_id, user_id),
            )

    def mark_completed(
        self, *, user_id: str, run_id: str, call_id: int,
        result_json: str, message: str, updated_at: str,
    ) -> None:
        with self._connection_factory() as conn:
            conn.execute(
                """
                UPDATE agent_tool_calls SET status='completed', result_json=%s
                WHERE id=%s AND run_id=%s AND user_id=%s
                """,
                (result_json, call_id, run_id, user_id),
            )
            conn.execute(
                """
                UPDATE agent_runs
                SET status='completed', result_json=%s, message=%s, updated_at=%s
                WHERE run_id=%s AND user_id=%s
                """,
