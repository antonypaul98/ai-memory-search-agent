...
class MemoryRepository:
    ...
    Document ID convention: {source_type}_{video_id}_{chunk_index}
    """
    ...
    def upsert_chunks(
        ...
        *,
        ...
        """
        Replace all chunks for an item with new embeddings and metadata.
        ...
        for chunk in chunks:
            ...
            page_number = (
                int(chunk.start_time_sec)
                if source_type == SourceType.PDF and chunk.start_time_sec >= 1
                else 0
            )
            meta: dict[str, Any] = {
    ...
    def search(
        ...
        *,
        ...
        """
        Query Chroma for the nearest chunk matches.

        Returns a flat list of hit dicts with metadata, document text, and score.
        """
        collection = get_collection(self._settings)
        ...
        for doc, meta, distance in zip(documents[0], metadatas[0], distances[0]):
            if meta is None:
                continue
            if not _metadata_belongs_to_user(meta, user_id):
                continue
    ...
    def delete_item(self, video_id: str, *, user_id: str = LOCAL_DEFAULT_USER_ID) -> None:
        """Remove all chunks belonging to a video for a user."""
        collection = get_collection(self._settings)
        try:
    ...
    def video_exists(self, video_id: str, *, user_id: str = LOCAL_DEFAULT_USER_ID) -> bool:
        """Return True if any chunks exist for the video for this user."""
        collection = get_collection(self._settings)
        try:
            result = collection.get(
                where={"$and": [{"video_id": video_id}, {"user_id": user_id}]},
                limit=1,
                include=[],
            )
